...
def build_recommendations(df: pd.DataFrame, model_info: dict,
    ...
    df2 = df[FEATURES + ["product_id", "category", "price",
                          "season", "festival"]].dropna(subset=FEATURES).copy()
    ...
    df2["pop_score"] = (
        0.40 * safe_norm(df2["units_sold"])
        + 0.25 * safe_norm(df2["interaction_score"])
        + 0.20 * safe_norm(df2["sentiment_score"] + 1)
        + 0.10 * df2["festival"].astype(float)
        + 0.05 * safe_norm(df2["sales_value"])
    )
    ...
    top = (df2.nlargest(top_n, "pop_score")[keep]
